...
const {
  ROOT_AUTH = '', ROOT_API= '', PROJECT_KEY = '', CLIENT_ID_CO = '' , CLIENT_SECRET_CO= '', SCOPE_CO ='' } = getConfig().publicRuntimeConfig as Record<string, string | undefined>;

export async function getTokenForCrosscheck(
  data?: UserAuthOptions
): Promise<Token> {
  let url = '';
  if(data) {
  ...
  const response = await fetch(url, {
    method: 'POST',
    headers: { Authorization: `Basic ${basicValue}` ,'Content-Type': 'application/x-www-form-urlencoded' },
  }).then(async (res) => {
    if (res.ok) {
      return res;
    }

    const {message, error} = await res.json() as {message: string | undefined, error: string | undefined};
...
export async function signInForCrosscheck(
  {username, password}: UserAuthOptions, accessToken: string
